...
function ScoreBoard() {
    const { previewSeconds, showPreviewCounter, selectionSeconds , misses , scores} = useContext(DataContext)
    ...
    return (
        ...
        <Box container sx={{ display: "flex", flexDirection: 'row', justifyContent: "center", height: "130px" }}>
            <Box sx={{ display: "flex", justifyContent: "center", alignContent: "center", pt: "5px", border: "thin solid rgb(23, 105, 170)", boxShadow: "0 0 10px rgba(23, 105, 170,0.7)", borderRadius: "15px", width: "150px", height: "50px", mt: "25px", }}>
                <Typography variant="h4" sx={{ color: "rgb(23, 105, 170)" }}>Score: {scores}</Typography>
            </Box>
            ...
            <Slide direction="up" in={showPreviewCounter} mountOnEnter unmountOnExit>
               <div> <ProgressTimer  value={(previewSeconds /(1+(3*scores) ) * 100)}  time={previewSeconds} title={"PREVIEW TIME"}  customcolor={"rgb(23, 105, 170)"} /> </div>
                </Slide>

                <Slide direction="down" in={!showPreviewCounter} mountOnEnter unmountOnExit>
               <div> <ProgressTimer  value={(selectionSeconds / (1+(5*scores)) * 100)}  time={selectionSeconds} title={" SELECTION TIME"} customcolor={"orangered"} /> </div>
                </Slide>
            </Box>
